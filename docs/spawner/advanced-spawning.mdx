...
1. First, go to your configuration and set the `busModelSpawnType` value to `"advanced"` <br/> ![Config Image with the busModelSpawnType](/img/docs/spawner/busModelSpawnTypeConfig.png)<br/> ![Config Image with the busModelSpawnType now set to "advanced"](/img/docs/spawner/busModelSpawnTypeConfigAdv.png)
2. Then, for this example we will be having our "model" be a "2015 Thomas C2" - For all buses that will just be using the same "2015 Thomas C2", go and set the `ModelName` attribute to "2015 Thomas C2". <br/>![Gif changing the Model Name](/img/docs/spawner/busModelName.gif)
3. Now, make sure the `BusLocations` Folder is ungrouped from the `WORKSPACE-UNGROUP` folder. Now you can move the arrow whereever you want it to go. The bus will spawn in the very center of that part. And the front of the bus is facing in the direction of the arrow. <br/>![Moving the Arrow](/img/docs/spawner/freeMovingArrow.gif)
...
6. Spawn your bus now with less lag. In this example we made the workspace part visible for you to see.
   ![Spawning a bus with advanced spawning](/img/docs/spawner/spawnBusAdvanced.gif)
